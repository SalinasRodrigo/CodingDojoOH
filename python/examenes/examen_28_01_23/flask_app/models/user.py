...
class User:
    ...
    def get_all(cls):
        ...
        emails_from_db =  connectToMySQL('thought_db').query_db(query)
        emails = []
    ...
    def get_user_thoughts(cls, data):
        query = ("SELECT * FROM users "+
                "LEFT JOIN thoughts ON users.id = thoughts.user_id "+
        ...
        for row_db in results:
            tought_data = {
                "id" : row_db["thoughts.id"],
                "contents" : row_db["contents"],
                "user_id": row_db["user_id"],
                "created_at" : row_db["thoughts.created_at"],
                "updated_at" : row_db["thoughts.updated_at"]
            }
            t = thought.Thought(tought_data)
            user.thoughts.append(t)
        return user
    ...
    def destroy(cls,id):
        ...
        return connectToMySQL('thought_db').query_db(query)
